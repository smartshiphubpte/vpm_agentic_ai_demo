"""Runtime configuration from process env or VPM_CONFIG_JSON."""
...
_STORM_CENTER_NM = float(
    vpm_cfg.get("VPM_STORM_CENTER_BUFFER_NM", "")
    or vpm_cfg.get("VPM_STORM_ROUTE_THRESHOLD_NM", "")
    or "500"
)
_STORM_EDGE_NM = float(vpm_cfg.get("VPM_STORM_EDGE_BUFFER_NM", "300"))
...
def _openai_model_from_env() -> str:
    ...
    explicit = vpm_cfg.get("OPENAI_MODEL", "").strip()
    if explicit:
        return explicit
    vpm = vpm_cfg.get("VPM_LLM_MODEL", "").strip()
...
class Settings:
    mode: str = vpm_cfg.get("VPM_MODE", "mock")  # mock | live
    base_url: str = vpm_cfg.get("VPM_BASE_URL", "http://localhost:3000/be_voyagepm")
    email: str = vpm_cfg.get("VPM_EMAIL", "ops@smartshiphub.com")
    password: str = vpm_cfg.get("VPM_PASSWORD", "demo")
    company: str = vpm_cfg.get("VPM_COMPANY", "orion")
    openai_api_key: str = vpm_cfg.get("OPENAI_API_KEY", "") or vpm_cfg.get("VPM_LLM_API_KEY", "")
    openai_model: str = _openai_model_from_env()
    gemini_api_key: str = vpm_cfg.get("GEMINI_API_KEY", "") or vpm_cfg.get("VPM_GEMINI_API_KEY", "")
    gemini_model: str = _gemini_model_from_env()
    gemini_base_url: str = (
        vpm_cfg.get("GEMINI_BASE_URL", "https://generativelanguage.googleapis.com/v1beta/openai")
        .rstrip("/")
    )
    cursor_api_key: str = vpm_cfg.get("CURSOR_API_KEY", "") or vpm_cfg.get("VPM_CURSOR_API_KEY", "")
    cursor_model: str = vpm_cfg.get("CURSOR_MODEL", "").strip() or vpm_cfg.get("VPM_CURSOR_MODEL", "").strip() or "composer-2.5"
    # local = cursor-sdk on this host; cloud = Cursor-hosted no-repo agent (better in Docker)
    cursor_runtime: str = vpm_cfg.get("VPM_CURSOR_RUNTIME", "local").strip().lower()
    # openai | openai_compatible | gemini | cursor
    llm_provider: str = vpm_cfg.get("VPM_LLM_PROVIDER", "openai")
    llm_base_url: str = vpm_cfg.get("VPM_LLM_BASE_URL", "").rstrip("/")
    # Live route optimize without voyagepm_be: conventional | llm | backend
    route_opt_method: str = vpm_cfg.get("VPM_ROUTE_OPT_METHOD", "conventional")
    # conventional only: astar | dijkstra
    route_opt_algo: str = vpm_cfg.get("VPM_ROUTE_OPT_ALGO", "astar")
    # Preferred standoff when placing graph nodes; land *crossing* is the hard reject
    land_clearance_nm: float = float(vpm_cfg.get("VPM_LAND_CLEARANCE_NM", "25"))
    ...
    tenant: str = vpm_cfg.get("VPM_TENANT", "").strip().lower()

    inbox_poll_seconds: float = float(vpm_cfg.get("VPM_INBOX_POLL_SECONDS", "30"))
    # IMAP mailbox — email + password is enough (iPowered defaults to imap.ipower.com).
    mail_imap_host: str = vpm_cfg.get("VPM_MAIL_IMAP_HOST", "").strip()
    mail_imap_port: int = int(vpm_cfg.get("VPM_MAIL_IMAP_PORT", "") or "993")
    mail_imap_user: str = (
        vpm_cfg.get("VPM_MAIL_IMAP_USER", "").strip()
        or vpm_cfg.get("VPM_MAIL_EMAIL", "").strip()
    )
    mail_imap_password: str = vpm_cfg.get("VPM_MAIL_IMAP_PASSWORD", "") or vpm_cfg.get(
        "VPM_MAIL_PASSWORD", ""
    )
    mail_imap_folder: str = vpm_cfg.get("VPM_MAIL_IMAP_FOLDER", "INBOX").strip() or "INBOX"
    mail_imap_ssl: bool = vpm_cfg.get("VPM_MAIL_IMAP_SSL", "true").lower() in ("1", "true", "yes")
    mail_reject_to: str = vpm_cfg.get("VPM_MAIL_REJECT_TO", "").strip()
    mail_subject_contains: str = vpm_cfg.get("VPM_MAIL_SUBJECT_CONTAINS", "").strip()
    # IMAP poll interval (seconds). Folder ingest still uses VPM_INBOX_POLL_SECONDS.
    mail_poll_seconds: float = float(vpm_cfg.get("VPM_MAIL_POLL_SECONDS", "") or "900")
    storm_interval_hours: float = float(vpm_cfg.get("VPM_STORM_INTERVAL_HOURS", "6"))
    noon_horizon_hours: float = float(vpm_cfg.get("VPM_NOON_HORIZON_HOURS", str(7 * 24)))
    waypoint_interval_hours: float = float(vpm_cfg.get("VPM_WAYPOINT_INTERVAL_HOURS", "6"))
    weather_report_delay_minutes: float = float(
        vpm_cfg.get("VPM_WEATHER_REPORT_DELAY_MINUTES", "0")
    )

    # Noon report polling (excel = drop folder; db = client std_enoonreporttable)
    noon_source: str = vpm_cfg.get("VPM_NOON_SOURCE", "excel")  # excel | db | both
    noon_excel_path: Path = _path("VPM_NOON_EXCEL_PATH", ROOT / "samples" / "noon_reports.xlsx")
    noon_poll_seconds: float = float(vpm_cfg.get("VPM_NOON_POLL_SECONDS", "120"))
    noon_batch_size: int = int(vpm_cfg.get("VPM_NOON_BATCH_SIZE", "1"))
    ...
    storm_source: str = vpm_cfg.get("VPM_STORM_SOURCE", "live")
    # live = Open-Meteo forecast+marine; backend = voyagepm_be /weather/route
    weather_source: str = vpm_cfg.get("VPM_WEATHER_SOURCE", "live")

    # Tagged daemon flow (see agents/specs/DaemonFlows.md)
    daemon_flow: str = vpm_cfg.get("VPM_DAEMON_FLOW", "pre_voyage_weather")
    # Heavy pool: route optimize, weather, storm (pollers stay independent)
    daemon_workers: int = int(vpm_cfg.get("VPM_DAEMON_WORKERS", "4"))
    # Ingest pool: inbox + noon pickup — separate so route-opt cannot starve ingest
    daemon_ingest_workers: int = int(vpm_cfg.get("VPM_DAEMON_INGEST_WORKERS", "2"))
    ...
    weather_report_on_noon: bool = vpm_cfg.get("VPM_WEATHER_REPORT_ON_NOON", "true").lower() in (
        "1",
        "true",
        "yes",
    )
    weather_report_on_prevoyage: bool = vpm_cfg.get(
        "VPM_WEATHER_REPORT_ON_PREVOYAGE", "false"
    ).lower() in ("1", "true", "yes")

    # Auto-email generated report PDFs. env = VPM_REPORT_EMAIL; db = placeholder (wire later)
    report_email: str = vpm_cfg.get("VPM_REPORT_EMAIL", "")
    report_email_source: str = vpm_cfg.get("VPM_REPORT_EMAIL_SOURCE", "env")  # env | db
    ...
    voyage_map_url: str = vpm_cfg.get("VPM_VOYAGE_MAP_URL", "")
    # Kick EOV report when an Arrival noon row is processed (runs in background thread)
    eov_on_arrival: bool = vpm_cfg.get("VPM_EOV_ON_ARRIVAL", "true").lower() in (
        "1",
        "true",
        "yes",
    )
